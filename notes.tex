...
\section{Methodology}
\label{sec:orgaf0dee1}

After explaning the insights and our new concepts around the metric of Quantum Volume, let us now
look at our methodology.
One issue that needs to be raised is the difficulty of \(\epsilon_{eff}\) calculation for a device.
...
with \(V_Q > V^a_Q\) and, obviously, \(N > n\).
...
mapping it.
We are interested in checking the relation between Quantum Volume and the probability of success, as
well.
